...
def get_all_results_dynamic(the_date,segment):
    resultsList = []
    for x in range(len(all_profiles)):
        results = get_results(service,all_profiles[x].get('items')[0].get('id'),segment,the_date,'ga:hour','ga:visits,ga:transactionRevenue')
        #print account_ids_to_websites[all_profiles[x].get('items')[0].get('webPropertyId')],'\n',results['rows'],'\n\n\n'
        resultsList.append(results)
    return resultsList
...
def get_segmented_results(the_date):
    resultsList = []
    for x in range(len(std.segmentsList)):
        segment = si.segments_Dictionary[std.segmentsList[x]]
        resultsList.append(get_all_results_dynamic(the_date,std.segmentsList[x]))
    return resultsList

def get_segmented_results_dynamic(the_date):
    resultsList = []
    for x in range(len(std.segmentsList)):
        segment = unicode('gaid::'+si.segments_Dictionary[std.segmentsList[x]])
...
def get_final_results():
    ...
    for the_date in dates:
        our_results = get_segmented_results_dynamic(the_date)
        final_results_list.append((our_results,the_date))
    return final_results_list
        
        
def write_to_file():
    filename = 'G:\.Customer Insight\%s'%(now.strftime('%Y-%m-%d')+'.txt')
    myNewFile = open(filename,'w+')
    for date in range(len(final_results)):
            for channel in range(len(final_results[date][0])):
                    for site in range (len(final_results[date][0][channel][0])):
                            for hour in range(len(final_results[date][0][channel][0][site]['rows'])):
                                    myNewFile.write(str(final_results[date][1])+'\t'+str(final_results[date][0][channel][1])+'\t'+str(final_results[date][0][channel][0][site]['profileInfo']['profileName'])+'\t'+str(final_results[date][0][channel][0][site]['rows'][hour][0])+'\t'+str(final_results[date][0][channel][0][site]['rows'][hour][1])+'\n')
    myNewFile.close()
